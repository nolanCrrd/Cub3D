...
/*   Updated: 2026/01/14 15:55:03 by ehode            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	load_rec(t_ctx *ctx)
{
	...
	while (i < 3)
	{
		ctx->rec[i] = mlx_new_image_from_file(ctx->mlx,
				(char *)rec_assets[i], NULL, NULL);
		if (!ctx->rec[i])
		{
...
void	display_rec(t_ctx *ctx)
{
	...
	if ((((size_t)ctx->frame * 10000) / 1000000) % 20 >= 15)
		mlx_put_transformed_image_to_window(ctx->mlx, ctx->win, ctx->rec[0],
			REC_X + 60, REC_Y + 40, 0.5, 0.5, 0);
	else if ((((size_t)ctx->frame * 10000) / 1000000) % 20 >= 10)
		mlx_put_transformed_image_to_window(ctx->mlx, ctx->win, ctx->rec[1],
			REC_X + 60, REC_Y + 40, 0.5, 0.5, 0);
	else if ((((size_t)ctx->frame * 10000) / 1000000) % 20 >= 5)
		mlx_put_transformed_image_to_window(ctx->mlx, ctx->win, ctx->rec[2],
			REC_X + 60, REC_Y + 40, 0.5, 0.5, 0);
	else
		mlx_put_transformed_image_to_window(ctx->mlx, ctx->win, ctx->rec[1],
			REC_X + 60, REC_Y + 40, 0.5, 0.5, 0);
